...
def dashboard(): #the homepage of sorts that is the first stop after logging in
    ...
    return render_template("homepage.html", user = User.get_user_by_id(user_data)) #homepage is dashboard for reference

@app.route("/new_route")
def extra_page(): #just takes us to our extra page we haven't decided on yet
    if "user_id" not in session:
        return redirect("/")
    user_data = {
        'id':session['user_id'],
    ...
    return render_template("extra_page.html", user = user)
