...
class User:
    ...
    def save(cls, user_data):
        ...
        return connectToMySQL("users_schema_dav").query_db(query, user_data)

    @classmethod
    def get_by_email(cls, user_data):
        query = "SELECT * FROM users WHERE email = %(email)s;"
        result = connectToMySQL("users_schema_dav").query_db(query,user_data)
...
# worry about this later
    @classmethod
    def delete_user(cls, user_data):
        query = "DELETE FROM users WHERE password = %(password)s"
        result = connectToMySQL("users_schema_dav").query_db(query,user_data)
